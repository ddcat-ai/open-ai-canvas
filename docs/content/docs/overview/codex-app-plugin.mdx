...
```text
请从本地源码安装 Infinite Canvas Codex 插件。
请把源码放到 ~/plugins/infinite-canvas，确认 plugins/infinite-canvas/.codex-plugin/plugin.json 存在，
...
```bash
mkdir -p ~/plugins
# 将当前项目源码放到 ~/plugins/infinite-canvas
```
...
```bash
node /path/to/infinite-canvas/canvas-agent/dist/index.js mcp
```
...
```bash
CANVAS_URL=http://localhost:3000 node /path/to/infinite-canvas/canvas-agent/dist/index.js
```
