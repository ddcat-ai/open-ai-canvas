...
func newCanvasHistoryTestService(t *testing.T) *Service {
	...
	if err := db.AutoMigrate(&model.CanvasProject{}, &model.CanvasCollaborator{}, &model.CanvasSnapshot{}, &model.CanvasSnapshotResource{}, &model.Resource{}, &model.Asset{}); err != nil {
		t.Fatal(err)
	}
	sqlDB, _ := db.DB()
