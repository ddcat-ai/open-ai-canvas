...
import (
	...
	"infinite-canvas/backend/internal/payment/wechatpay"
...
func (s *Service) payments() *payment.Registry {
	s.paymentMu.Lock()
	defer s.paymentMu.Unlock()
	if s.paymentRegistry == nil {
		s.paymentRegistry = payment.NewRegistry(wechatpay.New())
	}
