...
func (s *Service) FetchRunningHubAppInfo(ctx context.Context, req RunningHubWorkflowFetchRequest) (map[string]any, error) {
	...
	root := runningHubRootURL(config.BaseURL)
	endpoint := root + "/api/webapp/apiCallDemo?apiKey=" + url.QueryEscape(runningHubAPIKey(config)) + "&webappId=" + url.QueryEscape(webappID)
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodGet, endpoint, nil)
	if err != nil {
		return nil, err
	}
	httpReq.Header.Set("Accept", "application/json")
	ApplyOutboundHeaders(httpReq, config.Headers)
	data, _, err := doBinary(httpReq)
	if err != nil {
		return nil, fmt.Errorf("拉取 RunningHub App 参数失败：%w", err)
	}
	var response map[string]any
	if err := json.Unmarshal(data, &response); err != nil {
		return nil, errors.New("RunningHub App 响应不是有效 JSON")
	}
