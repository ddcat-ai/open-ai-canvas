...
describe("describeDirectorSaveStatus", () => {
  it("should report a quiet idle state only when everything is confirmed", () => {
    const indicator = describeDirectorSaveStatus(
      makeSnapshot({ status: "saved", revision: 3, confirmedRevision: 3 }),
    );
  ...
  it("should stay retryable when the status is saved but a revision is unconfirmed", () => {
    const indicator = describeDirectorSaveStatus(
      makeSnapshot({ status: "saved", revision: 4, confirmedRevision: 3 }),
    );
  ...
  it("should mark saving as busy and not retryable", () => {
    const indicator = describeDirectorSaveStatus(
      makeSnapshot({ status: "saving", revision: 2, confirmedRevision: 1 }),
    );
  ...
  it("should distinguish an error with a safe draft from one without", () => {
    const withDraft = describeDirectorSaveStatus(
      makeSnapshot({ status: "error", revision: 2, confirmedRevision: 1, draftStored: true }),
    );
    const withoutDraft = describeDirectorSaveStatus(
      makeSnapshot({ status: "error", revision: 2, confirmedRevision: 1, draftStored: false }),
    );
  ...
  it("should surface dirty as pending and retryable", () => {
    const indicator = describeDirectorSaveStatus(
      makeSnapshot({ status: "dirty", revision: 1, confirmedRevision: 0, draftStored: true }),
    );
    expect(indicator.label).toBe("有未保存修改");
    expect(indicator.retryable).toBe(true);
...
describe("shouldBlockDirectorUnload", () => {
  ...
  it("should block an error state that has no safe draft", () => {
    expect(
      shouldBlockDirectorUnload(
        makeSnapshot({ status: "error", revision: 1, confirmedRevision: 1, draftStored: false }),
      ),
    ).toBe(true);
  });
  ...
  it("should let an error state with a safe draft through", () => {
    expect(
      shouldBlockDirectorUnload(
        makeSnapshot({ status: "error", revision: 1, confirmedRevision: 1, draftStored: true }),
      ),
    ).toBe(false);
  });
...
describe("directorSaveProgress", () => {
  it("should project every progress field and drop the scene", () => {
    const progress = directorSaveProgress(
      makeSnapshot({ status: "saving", revision: 4, confirmedRevision: 2, draftStored: true }),
    );
