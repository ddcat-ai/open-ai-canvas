...
import {
  ...
  type DirectorFlushRequest,
  type FlushFunction,
  type StorageLike,
} from "../src/lib/canvas/director/director-save";
...
const makeEnvelope = (
  overrides: Partial<DirectorDraftEnvelope> = {},
...
const corruptEnvelope = (
  corrupt: (envelope: Record<string, unknown>) => void,
  overrides: Partial<DirectorDraftEnvelope> = {},
): string => {
...
const rawEnvelopeWith = (rawFields: Record<string, string>): string => {
  ...
  const rawEntries = Object.entries(rawFields).map(
    ([field, token]) => JSON.stringify(field) + ":" + token,
  );
...
const createHarness = (options: HarnessOptions = {}) => {
  ...
  const key = scopedStorageKey(
    "director-scene-draft:" + initialScene.id,
    scope,
  );
  ...
  const readEnvelope = (
    id: string = initialScene.id,
  ): DirectorDraftEnvelope | null => {
    const readKey =
      id === initialScene.id
        ? key
        : scopedStorageKey("director-scene-draft:" + id, scope);
