...
export default function CreatePage() {
    ...
    const handleLibrarySelect = (selected: Asset[]) => {
        const next = selected.flatMap((asset): CreationAttachment[] => {
            ...
            if (asset.kind === "audio" && mode === "video") return [creationAttachmentFromAudioAsset(asset)];
            return [];
        });
        if (!next.length) return;
    ...
    const submit = async (retryContext?: CreationRetryContext, retryLockKey?: string) => {
        ...
        // 页面附件在提交前按媒体类型拆分，shared generation task 继续负责本机与远端 provider 的统一协议。
    ...
    const cancelPendingMessage = async (item: CreationMessage) => {
        ...
        if (background && backgroundOutcome?.kind === "background") {
            updateConversationMessage(activeConversation.id, item.id, (message) => ({
                ...message,
                status: backgroundOutcome.creationStatus,
                generationStage: background.task.stage,
                content: backgroundOutcome.message,
            }));
            toast.info(backgroundOutcome.message);
            return;
        }
        ...
        if (cancelled.length === item.taskIds.length) {
            ...
            updateConversationMessage(activeConversation.id, item.id, (message) => ({
                ...message,
                status: outcome?.creationStatus ?? "cancelled",
                generationStage: "cancelled",
                content: outcome?.message ?? cancelled[0]!.error ?? "任务已取消",
            }));
        }
    };
...
function CreationAssetLibraryModal({
    ...
    const mediaAssets = useMemo(() => assets.filter((asset): asset is Extract<Asset, { kind: "image" | "video" | "audio" }> => asset.kind === "image" || asset.kind === "video" || asset.kind === "audio"), [assets]);
    ...
    for (const conversation of conversations) {
        for (const [messageIndex, message] of conversation.messages.entries()) {
            ...
            for (const [batchIndex, taskId] of message.taskIds.entries()) {
                contexts.set(taskId, {
                    prompt,
                    clientContext: { conversationId: conversation.id, messageId: message.id, batchIndex, batchCount: message.taskIds.length },
                });
            }
        }
    }
...
function creationTaskStageLabel(stage?: string) {
    if (stage === "queued") return "排队中";
    if (stage === "submitting") return "正在提交，等待官方确认：";
    if (stage === "submitted") return "已提交，等待状态更新：";
