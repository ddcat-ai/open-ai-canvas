...
// 文案全部走 auth catalog；eyebrow 两种语言都用大写英文装饰词，不进 catalog
const authEyebrows = {
    login: "WELCOME BACK",
...
export function AuthScene() {
    ...
    const page: keyof typeof authEyebrows = location.pathname === "/register" ? "register" : "login";
    const authTabs = [
    ...
    return (
        <main className="h-dvh min-h-0 overflow-y-auto bg-[#08090c] text-white lg:overflow-hidden">
            <div className="grid min-h-full lg:h-full lg:grid-cols-[minmax(0,1.32fr)_minmax(520px,1fr)]">
                <section className="relative min-h-[250px] overflow-hidden sm:min-h-[320px] lg:min-h-0" aria-label={t("hero.video-aria")}>
                ...
                <section className="relative flex min-h-[620px] items-start justify-center overflow-y-auto bg-[#0b0c10] px-4 pb-8 pt-20 sm:px-8 lg:min-h-0 lg:px-10 lg:pb-10 lg:pt-20">
                    <div className="absolute right-5 top-5 z-20 flex items-center gap-2 lg:right-8 lg:top-8">
                        <LocaleToggle className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white/58 backdrop-blur-xl transition hover:border-white/20 hover:bg-white/[0.08] hover:text-white" />
                        <Link to="/" className="inline-flex h-9 items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 text-xs text-white/58 backdrop-blur-xl transition hover:border-white/20 hover:bg-white/[0.08] hover:text-white">
                    ...
                    >
                        <ConfigProvider theme={getAntThemeConfig(true)}>
                            <div className="auth-card-dark h-auto overflow-hidden rounded-lg bg-[#121318]/94 shadow-[0_28px_80px_rgba(0,0,0,.34)] backdrop-blur-2xl">
                                <section aria-label={t(`${page}.title`)} className={`flex flex-col ${page === "login" ? "min-h-[500px]" : "min-h-[620px] sm:min-h-[640px]"}`}>
                                    <header className="px-6 pb-5 pt-6 sm:px-8 sm:pt-7">
                                        <p className="text-xs font-semibold tracking-[0.18em] text-blue-300/80">{authEyebrows[page]}</p>
                                        <h2 className="mt-2 text-3xl font-semibold">{t(`${page}.title`)}</h2>
                                        <p className="mt-2 text-sm leading-6 text-white/45">{t(`${page}.description`)}</p>
                                    </header>
                                    <div className="px-6 sm:px-8">
                                        <Tabs className="auth-card-tabs" activeKey={page} items={authTabs} onChange={(key) => navigate({ pathname: key === "register" ? "/register" : "/login", search: location.search })} />
                                    </div>
