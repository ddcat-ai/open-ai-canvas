                ...
                <aside className="workflow-preview-panel">
                    <header className="workflow-preview-header">
                        <div className="workflow-preview-header-row">
                            ...
                            {/* 合并说明：上游已把 antd Segmented 全量替换为自研 SegmentedControl（本文件不再 import Segmented），
                                故此处必须用 SegmentedControl；同时保留影策自有「链路」tab（previewTab 联合类型见第 93 行）。 */}
                            <SegmentedControl size="sm" value={previewTab} onChange={(value) => setPreviewTab(value as typeof previewTab)} options={[{ value: "latest", label: "最新" }, { value: "history", label: `历史 ${artifacts.length}` }, { value: "chain", label: "链路" }]} />
                        </div>
<SegmentedControl
                            block
                            size="sm"
