...
export default function SharedCanvasPage() {
    ...
    const [title, setTitle] = useState(t("canvas:shared-canvas"));
    ...
    const unauthorized = useCallback(() => message.warning(t("canvas:not-allowed-shared-canvases-are-view-only-this-action-was-not-executed")), [message]);
    ...
    useEffect(() => {
        ...
        getPublicCanvasShare(token)
            .then(({ project }) => {
                if (!active) return;
                setTitle(project.title || t("canvas:shared-canvas"));
            ...
            .catch((error) => {
                if (active) setLoadError(error instanceof Error ? error.message : t("canvas:share-link-is-invalid-or-expired"));
            })
            .finally(() => {
    ...
    const addNode = (type: CanvasNodeType) => {
        ...
        setNodes((current) => [
            ...current,
            {
                id: `shared-${nanoid()}`,
                type,
                title: type === CanvasNodeType.Text ? t("canvas:temporary-text") : type === CanvasNodeType.Image ? t("canvas:temporary-image") : type === CanvasNodeType.Video ? t("canvas:temporary-video") : t("canvas:temporary-storyboard"),
                position: contextMenu.world,
                width: size.width,
                height: size.height,
                metadata: type === CanvasNodeType.Text ? { content: t("canvas:this-node-exists-only-in-this-browser-page-and-disappears-on-refresh") } : {},
            },
        ]);
        setContextMenu(null);
        message.info(t("canvas:temporary-node-added-it-will-disappear-on-refresh"));
    };
    ...
    if (loading) return <FullScreenLoader label={t("canvas:opening-shared-canvas")} detail={t("canvas:loading-nodes-connections-and-view-state")} />;
    if (loadError)
        return (
            <div className="grid h-screen place-items-center px-5" style={{ background: theme.canvas.background }}>
                <WorkspaceState
                    icon="error"
                    title={t("canvas:share-link-unavailable")}
                    description={loadError}
                    action={
                        <Link to="/">
                            <Button>{t("canvas:back-to-home")}</Button>
                        </Link>
                    }
                />
            </div>
    ...
    return (
        <main className="relative h-screen overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            ...
            <CanvasNodeToolbar
                ...
                onTrimVideoRegenerate={unauthorized}
            ...
            <div className="pointer-events-none absolute bottom-5 right-5 z-[var(--z-panel-floating)] max-w-[340px] text-right text-xs leading-5" style={{ color: theme.node.muted }}>
                {t("canvas:guest-actions-apply-only-within-this-page")}
            </div>
...
function SharedContextMenu({ menu, onAdd, onInfo, onUnauthorized }: { menu: ContextMenu; onAdd: (type: CanvasNodeType) => void; onInfo: () => void; onUnauthorized: () => void }) {
    ...
    return (
        ...
        >
            {menu.nodeId ? (
                <>
                    <MenuButton icon={<Eye />} label={t("canvas:view-node-info")} onClick={onInfo} />
                    <MenuButton icon={<LockKeyhole />} label={t("canvas:edit-or-generate")} onClick={onUnauthorized} />
                </>
            ) : (
                <>
                    <div className="px-2 py-1.5 text-[var(--fs-label)]" style={{ color: theme.node.muted }}>
                        {t("canvas:add-temporary-node")}
                    </div>
                    <MenuButton icon={<FileText />} label={t("canvas:text-node")} onClick={() => onAdd(CanvasNodeType.Text)} />
                    <MenuButton icon={<ImageIcon />} label={t("canvas:image-node")} onClick={() => onAdd(CanvasNodeType.Image)} />
                    <MenuButton icon={<Video />} label={t("canvas:video-node")} onClick={() => onAdd(CanvasNodeType.Video)} />
                    <MenuButton icon={<Clapperboard />} label={t("canvas:storyboard-script")} onClick={() => onAdd(CanvasNodeType.Script)} />
                </>
            )}
        </div>
    );
    ...
    return (
        <div className="flex h-full w-full flex-col overflow-hidden rounded-[var(--panel-radius)]">
            ...
            <div className="min-h-0 flex-1 whitespace-pre-wrap break-words p-4 text-sm leading-6" style={{ color: theme.node.muted }}>
                {node.metadata?.composerContent || node.metadata?.prompt || t("canvas:prompt-not-filled-in")}
            </div>
        ...
        <div className="flex h-full w-full flex-col overflow-hidden rounded-[var(--panel-radius)]">
            <div className="flex h-10 shrink-0 items-center gap-2 border-b px-4" style={{ background: theme.node.panel, borderColor: theme.node.stroke }}>
                ...
                <button
                    ...
                    aria-label={t("canvas:one-click-video-nodes")}
                >
