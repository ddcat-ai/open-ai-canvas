...
import { buildActionBoardPrompt } from "./canvas-prompts";
...
export function useCanvasStoryboard({ projectId, nodesRef, connectionsRef, setNodes, setConnections, setSelectedNodeIds, enqueueGenerationBatch }: UseCanvasStoryboardOptions) {
    ...
    const confirmGenerationSubmission = useCallback(
        (count: number, model: string, taskLabel: string) =>
            new Promise<boolean>((resolve) => {
                ...
                modal.confirm({
                    title: t("canvas:confirm-submitting-param-param-tasks", { count: count, taskLabel: taskLabel }),
                    content: t("canvas:task-count-pricing-note", { count, model: modelDisplayName(effectiveConfig, model) }),
                    okText: t("canvas:confirm-generation"),
    ...
    const generateScriptRows = useCallback(
        async (nodeId: string, prompt: string) => {
            ...
            try {
                ...
                message.warning(error instanceof Error ? error.message : t("canvas:storyboard-context-incomplete"));
                return;
            }
            ...
            setNodes((current) =>
                current.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, composerContent: prompt, status: NODE_STATUS_LOADING, taskStage: t("canvas:creating-tasks"), taskProgress: 0, errorDetails: undefined } } : node)),
            );
            try {
                const task = await createGenerationTask({
                    ...
                    input: {
                        ...
                        requirements: t("canvas:produces-an-editable-storyboard-sheet-for-batch-image-and-video-generati"),
                ...
                message.success(t("canvas:generated-param-shots", { length: result.rows.length }));
                return true;
            } catch (error) {
                ...
                const imageNode = existing
                    ? {
                          ...
                          metadata: {
                              ...existingMetadata,
                              prompt,
                              ...storyboardPromptTemplateMetadata(row, "image"),
                              workflowKind: "shot" as const,
                              workflowTitle: t("canvas:shot-param-storyboard-frame", { shotNumber: row.shotNumber }),
                    ...
                    : createCanvasNode(
                          ...
                          { prompt, ...storyboardPromptTemplateMetadata(row, "image"), workflowKind: "shot", workflowTitle: t("canvas:shot-param-storyboard-frame", { shotNumber: row.shotNumber }), shotIndex: row.shotNumber, status: NODE_STATUS_IDLE },
                      );
                if (!existing) {
                    imageNode.title = t("canvas:shot-param-storyboard-frame-2", { shotNumber: row.shotNumber });
    ...
    const createScriptImageNodes = useCallback(
        (nodeId: string, rowIds?: string[]) => {
            ...
            if (missing.length) return message.warning(t("canvas:param-shots-lack-scene-descriptions-or-image-prompts", { length: missing.length }));
            ...
            message.success(createdCount ? t("canvas:created-param-image-nodes", { createdCount: createdCount }) : t("canvas:synced-prompts-of-existing-image-nodes"));
        },
    ...
    const generateScriptImages = useCallback(
        async (nodeId: string, rowIds: string[]) => {
            ...
            if (!targetRows.length) return message.info(t("canvas:selected-storyboard-frames-are-already-generated-or-generating"));
            if (!(await confirmGenerationSubmission(targetRows.length, imageModel, t("canvas:image-generation")))) return;
            ...
            if (
                ...
                message.success(t("canvas:storyboard-frames-queued-for-generation"));
        },
    ...
    const createScriptVideoNodes = useCallback(
        (nodeId: string, silent = false, rowIds?: string[]) => {
            ...
            rows.forEach((row, index) => {
                ...
                if (existingIndex >= 0) {
                    ...
                    nextNodes[existingIndex] = {
                        ...
                        metadata: {
                            ...
                            shotIndex: row.shotNumber,
                            workflowKind: "shot",
                            workflowTitle: t("canvas:shot-param-video", { shotNumber: row.shotNumber }),
                ...
                videoNode.title = t("canvas:shot-param-video-2", { shotNumber: row.shotNumber });
            ...
            setNodes(nextNodes);
            setConnections(nextConnections);
            if (!silent) message.success(createdCount ? t("canvas:created-param-video-nodes", { createdCount: createdCount }) : t("canvas:synced-prompts-of-existing-video-nodes"));
        },
    ...
    const createAndGenerateScriptVideos = useCallback(
        async (nodeId: string, rowIds?: string[]) => {
            ...
            if (!targetRows.length) {
                if (describedRows.some((row) => row.videoNodeId && nodesRef.current.some((node) => node.id === row.videoNodeId && Boolean(node.metadata?.content)))) message.info(t("canvas:shot-videos-already-exist"));
                else message.warning(t("canvas:fill-in-shot-scene-descriptions-first"));
                return;
            }
            if (!(await confirmGenerationSubmission(targetRows.length, videoModel, t("canvas:video-generation")))) return;
            ...
            if (
                ...
                message.success(t("canvas:shot-videos-queued-for-generation"));
        },
    ...
    const createScriptActionBoards = useCallback(
        async (nodeId: string) => {
            ...
            if (!actionBoardRows.length) {
                message.info(t("canvas:action-boards-already-exist"));
                return;
            }
            if (!(await confirmGenerationSubmission(actionBoardRows.length, imageModel, t("canvas:action-board-generation")))) return;
            const imageSpec = NODE_DEFAULT_SIZE[CanvasNodeType.Image];
            const startX = scriptNode.position.x + scriptNode.width + 120;
            const nextNodes = [...nodesRef.current];
            const nextConnections = [...connectionsRef.current];
            ...
            actionBoardRows.forEach((row, index) => {
                const prompt = buildActionBoardPrompt({ shotNumber: row.shotNumber, plotDescription: row.plotDescription || "", videoMotionPrompt: row.videoMotionPrompt || "", characterNames: row.characters.map((item) => item.characterName).join("、") })
                    .filter(Boolean)
                    .join("\n");
                ...
                const imageNode =
                    existingIndex >= 0
                        ...
                        : createCanvasNode(
                              CanvasNodeType.Image,
                              { x: startX + imageSpec.width / 2, y: scriptNode.position.y + index * (imageSpec.height + 36) + imageSpec.height / 2 },
                              { prompt, workflowKind: "action_board", workflowTitle: t("canvas:shot-param-action-board", { shotNumber: row.shotNumber }), shotIndex: row.shotNumber, actionBoardRows: 4, actionBoardColumns: 3, status: NODE_STATUS_IDLE },
                          );
                imageNode.title = t("canvas:shot-param-action-board-2", { shotNumber: row.shotNumber });
            ...
            if (
                ...
                message.success(t("canvas:action-boards-queued-for-generation"));
        },
    ...
    const generateScriptVideos = useCallback(
        async (nodeId: string, rowIds: string[]) => {
            ...
            if (!readyRows.length) return message.warning(t("canvas:generate-and-review-first-frames-for-the-selected-shots-first"));
            if (readyRows.length !== rows.length) return message.warning(t("canvas:selected-shots-missing-first-frame", { count: rows.length - readyRows.length }));
            const videoModel = effectiveConfig.videoModel || effectiveConfig.model;
            if (!isAiConfigReady(effectiveConfig, videoModel)) {
                navigateToSettings({ continueCreation: true });
                return;
            ...
            if (!targetRows.length) return message.info(t("canvas:selected-shot-videos-are-already-generated-or-generating"));
            if (!(await confirmGenerationSubmission(targetRows.length, videoModel, t("canvas:video-generation")))) return;
            createScriptVideoNodes(
                nodeId,
                true,
