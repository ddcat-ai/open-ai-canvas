...
export function useCanvasNodeOperations({
    ...
    const cloneDrawingForNode = useCallback(
        (source: CanvasNodeData, target: CanvasNodeData, failureMessage: string) => {
            ...
            void cloneCanvasDrawing(projectId, sourceDrawingId, targetDrawingId)
                .then((saved) => {
                    if (!saved) {
                        if (source.metadata?.drawingShapeCount) message.warning(t("canvas:original-drawing-not-found-on-this-machine-a-blank-copy-was-created"));
                        return;
                    }
    ...
    const createFolder = useCallback(
        (position?: Position, linked?: { id: string; projectId: string; title: string; style: CanvasFolderStyle; theme: CanvasFolderTheme; createdAt: string }) => {
            ...
            folder.title = linked?.title || t("canvas:my-files");
            ...
            message.success(linked ? t("canvas:asset-folder-placed-on-canvas-open-it-to-browse-contents") : t("canvas:folder-created-drag-any-non-container-node-into-it"));
        },
    ...
    const arrangeSelectedNodes = useCallback(
        (mode: "row" | "column" | "grid" | "flow") => {
            ...
            message.success(mode === "flow" ? t("canvas:arranged-by-connections") : t("canvas:selected-nodes-arranged"));
        },
    ...
    const alignSelectedNodes = useCallback(
        (mode: CanvasAlignmentMode) => {
            ...
            commitNodes(nodesRef.current.map((node) => (positions.has(node.id) ? { ...node, position: positions.get(node.id)! } : node)));
            message.success(mode === "distributeX" || mode === "distributeY" ? t("canvas:selected-nodes-distributed-evenly") : t("canvas:selected-nodes-aligned"));
        },
    ...
    const createStoryboardGroup = useCallback(() => {
        ...
        if (images.length < 2) {
            message.warning(t("canvas:select-at-least-two-existing-images"));
            return;
        }
        const gap = 24;
        const padding = 24;
        ...
        const frame = createCanvasNode(
            ...
            { x: left + frameWidth / 2 - padding, y: top + frameHeight / 2 - FRAME_HEADER_HEIGHT - padding },
            {
                ...
                workflowTitle: t("canvas:storyboard-group"),
                frame: { collapsed: false, expandedWidth: frameWidth, expandedHeight: frameHeight },
            },
        );
        frame.title = t("canvas:storyboard-group-param-shots", { length: images.length });
        ...
        message.success(t("canvas:created-a-storyboard-group-with-param-shots", { length: images.length }));
    ...
    const createReferenceGroup = useCallback(() => {
        const media = nodesRef.current
            ...
            .sort((a, b) => a.position.y - b.position.y || a.position.x - b.position.x);
        if (media.length < 2) {
            message.warning(t("canvas:select-at-least-two-existing-image-or-video-nodes"));
            return;
        }
        const gap = 20;
        const padding = 24;
        ...
        const frame = createCanvasNode(
            ...
            { x: left + frameWidth / 2, y: top + frameHeight / 2 },
            {
                ...
                workflowTitle: t("canvas:reference-group"),
                referenceAssetNodeIds: media.map((node) => node.id),
                frame: { collapsed: false, expandedWidth: frameWidth, expandedHeight: frameHeight },
            },
        );
        frame.title = t("canvas:reference-group-param-items", { length: media.length });
        frame.position = { x: left - padding, y: top - FRAME_HEADER_HEIGHT - padding };
        frame.width = frameWidth;
        frame.height = frameHeight;
        ...
        selectNodes(new Set([frame.id]));
        message.success(t("canvas:created-a-reference-group-with-param-items-collapse-it-to-use-as-a-route", { length: media.length }));
    }, [commitNodes, message, nodesRef, selectNodes, selectedNodeIdsRef]);
    ...
    const toggleNodeLocked = useCallback(
        (nodeId: string) => {
            ...
            message.success(locked ? t("canvas:node-position-and-size-locked") : t("canvas:node-unlocked"));
        },
        ...
        (nodeId: string) => {
            ...
            const copiedNodes = sources.map((node) => {
                ...
                return {
                    ...
                    title: node.id === source.id ? `${node.title.replace(/ · [A-Z]$/, "")} · ${versionLabel || t("canvas:copy")}` : node.title,
            ...
            copiedNodes
                ...
                .forEach((targetNode) => {
                    ...
                    if (sourceNode) cloneDrawingForNode(sourceNode, targetNode, t("canvas:failed-to-save-drawing-copy-please-copy-again"));
                });
    ...
    const setPrimaryVersion = useCallback(
        (nodeId: string) => {
            ...
            message.success(t("canvas:set-as-primary-version", { name: target.metadata?.versionLabel || target.title }));
        },
        [commitNodes, message, nodesRef],
    );
    ...
    const pasteCopiedNodes = useCallback(
        (position?: Position) => {
            ...
            nextNodes
                ...
                .forEach((targetNode) => {
                    const sourceNode = sourceByTargetId.get(targetNode.id);
                    if (sourceNode) cloneDrawingForNode(sourceNode, targetNode, t("canvas:failed-to-save-drawing-copy-please-paste-again"));
                });
