...
export function useCanvasNodeEditor({ canvasId, domainProjectId, nodesRef, setNodes, setSelectedNodeIds, setSelectedConnectionId, setDialogNodeId, setToolbarNodeId, setHoveredNodeId }: UseCanvasNodeEditorOptions) {
        ...
        (nodeId: string, patch: Partial<CanvasNodeMetadata>) => {
            ...
            void ensureCanvasNodeAsset({ canvasId, domainProjectId, node: updatedNode, source: "canvas-manual", category: patch.assetCategory })
                .then(async (result) => {
                    ...
                    message.success(t("canvas:asset-category-updated"));
                })
                .catch((error) => message.error(error instanceof Error ? error.message : t("canvas:failed-to-update-asset-category")));
        },
    ...
    const saveNodeAsset = useCallback(
        async (node: CanvasNodeData) => {
            if (node.type !== CanvasNodeType.Text && node.type !== CanvasNodeType.Image && node.type !== CanvasNodeType.Video && node.type !== CanvasNodeType.Audio) return message.error(t("canvas:this-node-type-cannot-be-saved-as-an-asset"));
            if (!node.metadata?.content?.trim()) return message.error(t("canvas:this-node-has-no-content-to-save"));
            try {
                ...
                if (domainProjectId) await queryClient.invalidateQueries({ queryKey: ["project", domainProjectId] });
                message.success(result.linkedToProject ? t("canvas:added-to-project-assets") : t("canvas:added-to-my-assets"));
            } catch (error) {
                message.error(error instanceof Error ? error.message : t("canvas:failed-to-save-asset"));
            }
        },
