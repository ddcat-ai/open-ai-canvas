...
import { chapterWorkflowDescription } from "./canvas-prompts";
...
export function useCanvasUpload({ canvasId, domainProjectId, nodesRef, selectedNodeIdsRef, getCanvasCenter, screenToCanvas, setNodes, setSelectedNodeIds, setSelectedConnectionId, setContextMenu, setDialogNodeId }: UseCanvasUploadOptions) {
    ...
    const persistMediaNode = useCallback(
        async (node: CanvasNodeData) => {
            try {
                ...
                message.warning(error instanceof Error ? t("canvas:media-added-to-canvas-but-asset-sync-failed-param", { message: error.message }) : t("canvas:media-added-to-canvas-but-asset-sync-failed"));
                return false;
            }
        },
    ...
    const createImageFileNode = useCallback(
        async (file: File, position: Position) => {
            const progress = startUploadStatus(t("canvas:upload-image"), t("canvas:reading-image-file"), domainProjectId ? 4 : 3);
            try {
                progress.update(t("canvas:uploading-to-server-and-syncing-resources"), 2);
                const image = await uploadImage(file);
                progress.update(t("canvas:updating-canvas-node"), 3);
                ...
                if (domainProjectId) progress.update(t("canvas:writing-project-assets"), 4);
                const persisted = await persistMediaNode(node);
                progress.done(persisted ? t("canvas:image-added-to-canvas") : t("canvas:image-added-project-asset-sync-will-retry"));
                return id;
            } catch (error) {
                const details = error instanceof Error ? error.message : t("canvas:image-upload-failed");
    ...
    const createImageAssetNode = useCallback(
        async (asset: ImageAsset, position?: Position) => {
            try {
                ...
                if (!content) {
                    message.error(t("canvas:asset-image-unavailable"));
                    return;
                }
                ...
                const node: CanvasNodeData = {
                    id,
                    type: CanvasNodeType.Image,
                    title: asset.title || t("canvas:asset-image"),
                ...
                message.error(error instanceof Error ? error.message : t("canvas:failed-to-read-asset-image"));
            }
        },
    ...
    const createVideoFileNode = useCallback(
        async (file: File, position: Position) => {
            const progress = startUploadStatus(t("canvas:upload-video"), t("canvas:reading-video-file"), domainProjectId ? 4 : 3);
            try {
                progress.update(t("canvas:uploading-to-server-and-syncing-resources"), 2);
                ...
                if (domainProjectId) progress.update(t("canvas:writing-project-assets"), 4);
                const persisted = await persistMediaNode(node);
                progress.done(persisted ? t("canvas:video-added-to-canvas") : t("canvas:video-added-project-asset-sync-will-retry"));
                return id;
            } catch (error) {
                const details = error instanceof Error ? error.message : t("canvas:video-upload-failed");
                progress.fail(details);
                message.error(details);
                return null;
            }
    ...
    const createAudioFileNode = useCallback(
        async (file: File, position: Position) => {
            const progress = startUploadStatus(t("canvas:upload-audio"), t("canvas:reading-audio-file"), domainProjectId ? 4 : 3);
            try {
                progress.update(t("canvas:uploading-to-server-and-syncing-resources"), 2);
                ...
                if (domainProjectId) progress.update(t("canvas:writing-project-assets"), 4);
                const persisted = await persistMediaNode(node);
                progress.done(persisted ? t("canvas:audio-added-to-canvas") : t("canvas:audio-added-project-asset-sync-will-retry"));
                return id;
            } catch (error) {
                const details = error instanceof Error ? error.message : t("canvas:audio-upload-failed");
                progress.fail(details);
                message.error(details);
                return null;
            }
    ...
    const createTextNodeFromClipboard = useCallback(
        (text: string, position?: Position) => {
            ...
            const node = {
                ...
                title: trimmed.slice(0, 32) || t("canvas:clipboard-text"),
            };
            setNodes((current) => [...current, node]);
        ...
        async (chapter: CanvasProjectChapterPayload, position?: Position) => {
            ...
            if (sourceText === undefined) {
                try {
                    ...
                    message.error(error instanceof Error ? t("canvas:failed-to-read-chapter-content-param", { message: error.message }) : t("canvas:failed-to-read-chapter-content"));
                    return;
                }
            }
            ...
            const node = createCanvasNode(CanvasNodeType.Text, position || getCanvasCenter(), {
                ...
                status: NODE_STATUS_SUCCESS,
                workflowKind: "free",
                workflowTitle: t("canvas:project-chapters"),
                workflowDescription: chapterWorkflowDescription(chapter.position + 1),
                chapterId: chapter.id,
                chapterTitle: chapter.title,
                fontSize: 14,
            });
            node.title = t("canvas:chapter-param", { title: chapter.title });
            node.width = 460;
            node.height = 280;
            setNodes((current) => [...current, node]);
            selectInsertedNode(node.id, "preserve");
            setContextMenu(null);
            message.success(t("canvas:added-param", { title: chapter.title }));
        },
    ...
    const handleUploadFiles = useCallback(
        async (files: File[]) => {
            ...
            if (!supportedFiles.length) {
                message.warning(t("canvas:choose-image-video-mp3-or-wav-files"));
                return false;
            }
            ...
            if (failedCount) message.warning(t("canvas:added-param-files-param-uploads-failed", { length: createdIds.length, failedCount: failedCount }));
            else message.success(t("canvas:added-param-files-to-canvas", { length: createdIds.length }));
            return true;
        },
    ...
    const uploadTimelineMedia = useCallback(
        async (files: File[]): Promise<TimelineDirectMedia[]> => {
            ...
            if (!supportedFiles.length) {
                message.warning(t("canvas:choose-video-mp3-or-wav-files"));
                return [];
            }
            ...
            for (const file of supportedFiles) {
                try {
                    ...
                    message.error(error instanceof Error ? t("canvas:asset-upload-failed-param", { message: error.message }) : t("canvas:asset-upload-failed"));
                }
            }
            if (created.length) message.success(t("canvas:uploaded-param-assets-to-the-timeline", { length: created.length }));
            return created;
        },
        [message],
    );
    ...
    const createVideoNodeFromBlob = useCallback(
        async (blob: Blob, title: string): Promise<CanvasNodeData | null> => {
            const progress = startUploadStatus(t("canvas:compose-video-segments"), t("canvas:upload-composed-result"), domainProjectId ? 4 : 3);
            try {
                progress.update(t("canvas:uploading-to-server-and-syncing-resources"), 2);
                ...
                if (domainProjectId) progress.update(t("canvas:writing-project-assets"), 4);
                const persisted = await persistMediaNode(node);
                progress.done(persisted ? t("canvas:new-video-segment-generated-and-added-to-project-assets") : t("canvas:new-video-segment-generated-project-asset-sync-will-retry"));
                return node;
            } catch (error) {
                const details = error instanceof Error ? error.message : t("canvas:failed-to-compose-video-segments");
                progress.fail(details);
                message.error(details);
                return null;
            }
    ...
    const replaceNodeMedia = useCallback(
        async (nodeId: string, file: File) => {
            ...
            if (isAudioFile(file)) {
                const progress = startUploadStatus(t("canvas:replace-audio"), t("canvas:reading-audio-file"));
                try {
                    progress.update(t("canvas:uploading-to-server-and-syncing-resources"), 2);
                    const audio = await uploadMediaFile(file, "audio");
                    progress.update(t("canvas:updating-canvas-node"), 3);
                    ...
                    const persisted = await persistMediaNode(node);
                    progress.done(persisted ? t("canvas:audio-replaced-undo-to-restore") : t("canvas:audio-replaced-project-asset-sync-will-retry"));
                    return true;
                } catch (error) {
                    const details = error instanceof Error ? error.message : t("canvas:audio-replacement-failed");
                    progress.fail(details);
                    message.error(details);
                    return false;
                }
            ...
            if (file.type.startsWith("video/")) {
                const progress = startUploadStatus(t("canvas:replace-video"), t("canvas:reading-video-file"));
                try {
                    progress.update(t("canvas:uploading-to-server-and-syncing-resources"), 2);
                    const video = await uploadMediaFile(file, "video");
                    progress.update(t("canvas:updating-canvas-node"), 3);
                    ...
                    const persisted = await persistMediaNode(node);
                    progress.done(persisted ? t("canvas:video-replaced-undo-to-restore") : t("canvas:video-replaced-project-asset-sync-will-retry"));
                    return true;
                } catch (error) {
                    const details = error instanceof Error ? error.message : t("canvas:video-replacement-failed");
                    progress.fail(details);
                    message.error(details);
                    return false;
                }
            }
            const progress = startUploadStatus(t("canvas:replace-image"), t("canvas:reading-image-file"));
            try {
                progress.update(t("canvas:uploading-to-server-and-syncing-resources"), 2);
                const image = await uploadImage(file);
                progress.update(t("canvas:updating-canvas-node"), 3);
                ...
                selectInsertedNode(nodeId, "open");
                const persisted = await persistMediaNode(node);
                progress.done(persisted ? t("canvas:image-replaced-undo-to-restore") : t("canvas:image-replaced-project-asset-sync-will-retry"));
                return true;
            } catch (error) {
                const details = error instanceof Error ? error.message : t("canvas:image-replacement-failed");
                progress.fail(details);
                message.error(details);
                return false;
            }
        ...
        async (position?: Position, clipboardEvent?: ClipboardEvent | null) => {
            ...
            const pasteImageFile = async (file: File) => {
                ...
                if (selected.length === 1 && selected[0].type === CanvasNodeType.Image) {
                    if (await replaceNodeMedia(selected[0].id, file)) message.success(t("canvas:replaced-with-clipboard-image-undo-to-restore"));
                    return true;
                }
                const inserted = await createImageFileNode(file, position || getCanvasCenter());
                if (inserted) message.success(t("canvas:image-added-from-clipboard"));
            ...
            try {
                ...
                if (createTextNodeFromClipboard(text, position)) {
                    message.success(t("canvas:text-added-from-clipboard"));
                    return true;
                }
            } catch {
                // ignore
        ...
        async (event: ChangeEvent<HTMLInputElement>) => {
            ...
            try {
                ...
                if (target?.nodeId) {
                    ...
                    if (!compatible) {
                        message.warning(t("canvas:choose-media-files-matching-the-current-node-type"));
                        return;
                    }
    ...
    const handleDrop = useCallback(
        (event: DragEvent<HTMLDivElement>) => {
            ...
            if (imageAssetId) {
                ...
                if (!asset) {
                    message.warning(t("canvas:asset-does-not-exist"));
                    return;
                }
            ...
            if (target) {
                void replaceNodeMedia(target.id, file).then((replaced) => {
                    if (replaced) message.success(t("canvas:media-replaced-undo-to-restore"));
                });
                return;
            }
    ...
    const handleAssetInsert = useCallback(
        async (payload: InsertAssetPayload, options: { openDialog?: boolean } = {}): Promise<CanvasNodeData | null> => {
            ...
            try {
                ...
                message.error(error instanceof Error ? error.message : t("canvas:failed-to-insert-asset"));
                return null;
            }
        },
    ...
    const handleProjectAssetsInsert = useCallback(
        async (payloads: InsertAssetPayload[], position?: Position): Promise<CanvasNodeData[]> => {
            ...
            try {
                ...
                setSelectedConnectionId(null);
                setDialogNodeId(null);
                message.success(t("canvas:imported-param-project-assets", { length: created.length }));
                return created;
            } catch (error) {
                message.error(error instanceof Error ? error.message : t("canvas:failed-to-import-project-assets"));
                throw error;
            }
        },
