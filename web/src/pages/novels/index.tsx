...
const genreOptions = ["古装", "都市", "年代", "校园", "家庭伦理", "悬疑", "奇幻", "重生逆袭", "复仇反杀", "甜宠爱情", "豪门恩怨", "职场成长", "轻喜剧"].map((value) => ({ label: value, value }));

const audienceOptions = ["女性情感向", "女性爽感向", "男性逆袭向", "年轻都市人群", "成熟情感人群", "家庭共看人群", "悬疑反转偏好", "轻松解压偏好"].map((value) => ({ label: value, value }));

const endingDirectionOptions = ["圆满反杀", "苦尽甘来", "携手成长", "开放式代价", "悲剧余韵", "悬念留白"].map((value) => ({ label: value, value }));
    ...
    return (
        <WorkspacePage className="library-page" grid>
            <PageHeader
                ...
                actions={
                    <Button type="primary" icon={<Plus className="size-3.5" />} onClick={() => setComposerOpen(true)}>
                        新建创作
                    </Button>
                }
            />
            ...
            {!runsQuery.isPending && !runsQuery.isError && !runs.length ? (
                <section className="mt-4 flex min-h-72 flex-col items-center justify-center border border-dashed border-foreground/15 px-5 text-center">
                    <span className="grid size-11 place-items-center rounded-md bg-foreground/5 text-foreground/65">
                        <BookOpenText className="size-5" />
                    </span>
                    <h2 className="mt-4 text-base font-semibold">开始一个作品</h2>
                    <Button className="mt-4" type="primary" icon={<Sparkles className="size-3.5" />} onClick={() => setComposerOpen(true)}>
                        建立总控档案
                    </Button>
                </section>
            ) : null}
            {runs.length ? (
                <section className="mt-4 grid gap-3 xl:grid-cols-2" aria-label="创作项目列表">
                    {runs.map((item) => (
                        <RunCard
                            key={item.run.id}
                            item={item}
            ...
            <Drawer
                ...
                size="large"
                open={composerOpen}
                onClose={() => !createMutation.isPending && setComposerOpen(false)}
                extra={
                    <Button type="primary" icon={<Play className="size-3.5" />} loading={createMutation.isPending} onClick={start}>
                        开始创作
                    </Button>
                }
            >
                <div className="space-y-5 pb-8">
                    ...
                    />
                    <Field label="作品名">
                        <Input value={draft.projectName} maxLength={60} placeholder="可留空，由故事起点生成" onChange={(event) => updateDraft("projectName", event.target.value)} />
                    </Field>
                    <Field label="故事起点" required>
                        <Input.TextArea value={draft.premise} autoSize={{ minRows: 5, maxRows: 10 }} placeholder="人物、想得到什么、阻力和最重要的秘密。" onChange={(event) => updateDraft("premise", event.target.value)} />
                    </Field>
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Field label={`题材（最多 ${GENRE_SELECTION_LIMIT} 项）`}>
                            <Select
                                className="w-full"
                                mode="multiple"
                                maxCount={GENRE_SELECTION_LIMIT}
                                value={draft.genre}
                                placeholder="选一个主类型，再补充一到两个卖点"
                                options={genreOptions}
                                onChange={(values) => updateDraft("genre", values as string[])}
                            />
                        </Field>
                        <Field label={`目标${unitLabel(draft.outputMode)}数`}>
                            <InputNumber className="w-full" min={1} max={500} value={draft.targetUnitCount} onChange={(value) => updateDraft("targetUnitCount", Number(value || 1))} />
                        </Field>
                        <Field label={`单${unitLabel(draft.outputMode)}目标字数`}>
                            <InputNumber className="w-full" min={200} max={12_000} step={100} value={draft.targetUnitLength} onChange={(value) => updateDraft("targetUnitLength", Number(value || 200))} />
                        </Field>
                        {draft.outputMode === "screenplay" ? (
                            <Field label="单集目标时长">
                                <InputNumber className="w-full" min={15} max={1_800} step={15} addonAfter="秒" value={draft.unitDurationSeconds} onChange={(value) => updateDraft("unitDurationSeconds", Number(value || 15))} />
                            </Field>
                        ) : null}
                        <Field label={`受众（最多 ${AUDIENCE_SELECTION_LIMIT} 项）`}>
                            <Select
                                className="w-full"
                                mode="multiple"
                                maxCount={AUDIENCE_SELECTION_LIMIT}
                                value={draft.audience}
                                placeholder="先选核心受众，必要时再选次受众"
                                options={audienceOptions}
                                onChange={(values) => updateDraft("audience", values as string[])}
                            />
                        </Field>
                    </div>
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Field label="叙事结构">
                            <Select
                                className="w-full"
                                value={draft.structurePreference || undefined}
                                placeholder="选择或留空"
                                allowClear
                                onChange={(value) => updateDraft("structurePreference", value || "")}
                                options={["单线推进", "双线并行", "多线群像", "悬念反转"].map((value) => ({ label: value, value }))}
                            />
                        </Field>
                        <Field label="整体基调">
                            <Select
                                ...
                                options={["强情绪", "甜宠治愈", "紧张悬疑", "轻喜剧", "现实向"].map((value) => ({ label: value, value }))}
                            />
                        </Field>
                    </div>
                    <Field label="结局方向">
                        <Select className="w-full" value={draft.endingDirection || undefined} placeholder="选择或留空" allowClear options={endingDirectionOptions} onChange={(value) => updateDraft("endingDirection", value || "")} />
                    </Field>
                    <Field label="额外要求">
                        <Input.TextArea value={draft.customRequirements} autoSize={{ minRows: 2, maxRows: 5 }} placeholder="人物关系、禁忌、节奏或其他不能违背的设定。" onChange={(event) => updateDraft("customRequirements", event.target.value)} />
                    </Field>
                    <div className="border-t border-foreground/10 pt-4">
                        <div className="mb-2 flex items-center justify-between gap-3">
                            <span className="text-sm font-medium">文本模型</span>
                            <button type="button" className="text-xs text-foreground/55 underline-offset-2 hover:underline" onClick={() => navigate(settingsPath("models"))}>
                                模型设置
                            </button>
                        </div>
            ...
            <div className="flex min-w-0 items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0">
                        <h2 className="truncate text-sm font-semibold">{primaryTitle}</h2>
                        <p className="mt-0.5 text-xs text-foreground/50">{modeLabel(run.outputMode)}</p>
                    </div>
                </div>
            ...
            <div className="mt-4 grid grid-cols-3 divide-x divide-foreground/10 border-y border-foreground/10 py-2.5 text-center">
                <div>
                    <div className="text-sm font-semibold">
                        {run.completedUnitCount}/{run.targetUnitCount}
                    </div>
                    <div className="mt-0.5 text-[11px] text-foreground/48">已完成{label}</div>
                </div>
                <div>
                    <div className="truncate px-2 text-sm font-semibold">{run.status === "completed" ? "完成" : `第 ${nextUnit} ${label}`}</div>
                    <div className="mt-0.5 text-[11px] text-foreground/48">当前进度</div>
                </div>
                <Tooltip title={currentArc || "总控档案生成后显示"}>
                    <div>
                        <div className="truncate px-2 text-sm font-semibold">{currentArc || "准备中"}</div>
                        <div className="mt-0.5 text-[11px] text-foreground/48">当前分部</div>
                    </div>
                </Tooltip>
            </div>
            <div className="mt-3 flex items-center justify-between gap-2">
                <span className="min-w-0 truncate text-xs text-foreground/48">{statusNote}</span>
                <div className="flex shrink-0 items-center gap-1.5">
                    ...
                    {run.status === "paused" || run.status === "failed" ? (
                        <Button size="small" type="primary" icon={<Play className="size-3.5" />} loading={resuming} onClick={onResume}>
                            恢复
                        </Button>
                    ) : null}
                </div>
