...
export function summarizeCanvasContext(nodes: CanvasNodeData[], selectedNodeIds: Set<string>, units: ProjectUnit[] = []): CanvasContextSummary {
    ...
    return {
        ...
        chapterLabel: chapterTitles.length === 1 ? chapterTitles[0] : chapterTitles.length > 1 ? t("canvas:param-chapters", { length: chapterTitles.length }) : "",
        shotLabel: shotIndexes.length === 1 ? `镜头 ${shotIndexes[0] + 1}` : shotIndexes.length > 1 ? t("canvas:param-shots", { length: shotIndexes.length }) : "",
    };
}
