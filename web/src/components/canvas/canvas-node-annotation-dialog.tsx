        ...
        <Modal title={null} open={open} onCancel={onClose} footer={null} width="min(1120px, calc(100vw - 32px))" centered destroyOnHidden>
            <div className="flex flex-col gap-3">
                <div className="flex flex-wrap items-center gap-2 rounded-lg border p-2" style={{ borderColor: "rgba(127,127,127,.22)" }}>
                    <span className="px-1 text-sm font-semibold">{t("domain:annotate-2")}</span>
                    <span className="mx-1 h-6 w-px bg-current opacity-15" />
                    <ToolButton title={t("domain:brush-2")} active={mode === "brush"} onClick={() => setMode("brush")}>
                        <Brush className="size-4" />
                    </ToolButton>
                    <ToolButton title={t("domain:eraser")} active={mode === "erase"} onClick={() => setMode("erase")}>
                    ...
                    <div className="flex items-center gap-1 px-1">
                        {colors.map((item) => (
                            <button
                                key={item}
                                type="button"
                                aria-label={t("domain:color-param", { item: item })}
                    ...
                    <span className="mx-1 h-6 w-px bg-current opacity-15" />
                    <ToolButton title={t("canvas:undo")} disabled={!strokes.length} onClick={undo}>
                        <Undo2 className="size-4" />
                    </ToolButton>
                    <ToolButton title={t("canvas:redo")} disabled={!redoStrokes.length} onClick={redo}>
                        <Redo2 className="size-4" />
                    </ToolButton>
                    <ToolButton
                        title={t("canvas:clear-4")}
                ...
                <div className="flex min-h-[360px] items-center justify-center overflow-hidden rounded-lg bg-black/5 dark:bg-white/[0.03]">
                    {source && size.width ? (
                        <div className="relative inline-block max-h-[72vh] max-w-full overflow-hidden">
                            <img src={source} alt={t("domain:image-to-annotate")} className="block max-h-[72vh] max-w-full select-none object-contain" draggable={false} />
                            <canvas
                                ref={canvasRef}
                                width={size.width}
                                height={size.height}
                        ...
                        <span className="text-sm opacity-50">{t("domain:reading-image")}</span>
                    )}
                </div>
            </div>
        </Modal>
