            ...
            <div className="thin-scrollbar grid max-h-[70vh] grid-flow-col auto-cols-[328px] gap-2.5 overflow-x-auto pb-1.5">
                {versions.map((node) => {
                    ...
                    <article key={node.id} className="overflow-hidden rounded-[var(--r-lg)] border" style={{ borderColor: node.metadata?.versionPrimary ? theme.accent.primary : theme.node.stroke, background: theme.node.panel, boxShadow: node.metadata?.versionPrimary ? "0 0 0 2px " + theme.accent.primarySoft : undefined }}>
                        ...
                        <button type="button" className="block h-48 w-full overflow-hidden" style={{ background: theme.node.fill }} onClick={() => onFocus(node.id)}>
                        ...
                        <div className="flex flex-col gap-1.5 p-2.5 text-[var(--fs-label)]">
                            <Info label="模型" value={node.metadata?.model || "默认模型"} />
                            <Info label="尺寸" value={node.metadata?.size || "默认尺寸"} />
                            <div><div className="whitespace-nowrap opacity-45">提示词</div><div className="mt-0.5 line-clamp-5 whitespace-pre-wrap break-words leading-5">{node.metadata?.composerContent || node.metadata?.prompt || "未填写"}</div></div>
...
function Info({ label, value }: { label: string; value: string }) {
    return <div className="flex min-w-0 items-center gap-2"><span className="w-9 shrink-0 whitespace-nowrap opacity-45">{label}</span><span className="min-w-0 flex-1 truncate text-right font-medium" title={value}>{value}</span></div>;
}
