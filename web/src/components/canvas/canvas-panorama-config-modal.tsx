    ...
    <div className="grid min-h-[560px] grid-cols-[270px_minmax(0,1fr)_330px]">
            ...
            {selectedReferences.map((image, index) => (
              <div key={image.id} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] p-1.5">
                ...
                <div className="mt-1 flex items-center gap-1">
                  ...
                  <div className="min-w-0 flex-1 truncate text-[10px] text-[var(--color-text)]">{sourceMode === 'image' ? '源图' : `图${index + 1}`} · {image.label}</div>
                  <button
                    type="button"
      ...
      <aside className="flex flex-col gap-4 border-l border-[var(--color-border)] bg-[var(--color-bg-subtle)] p-4">
        ...
        {projection === 'cylindrical' && (
          <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-2.5 py-1.5 text-[11px] leading-5 text-amber-700 dark:text-amber-200/85">
          ...
          <div className="w-[560px] max-w-[calc(100%-32px)] rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] shadow-2xl">
            ...
            <div className="ui-scrollbar max-h-[380px] overflow-y-auto p-4">
              {filteredReferences.length === 0 ? (
                <div className="rounded-lg border border-dashed border-[var(--color-border)] px-4 py-10 text-center text-xs text-[var(--color-text-muted)]">
                  没有匹配的图片资产
                </div>
              ) : (
            ...
            <div className="flex justify-end gap-2 border-t border-[var(--color-border)] px-4 py-3">
              <button type="button" onClick={() => setIsAssetPickerOpen(false)} className="rounded-md border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-1.5 text-xs text-[var(--color-text-muted)] hover:bg-[var(--color-bg-hover)]">
                取消
              </button>
              <button
                type="button"
...
export function CanvasPanoramaConfigModal({
  open,
  onCancel,
  onConfirm,
  onCopyPrompt,
  ...
  initialSmartBase,
  nodes,
}: CanvasPanoramaConfigModalProps) {
  return (
    <AppModal
      title="全景图生成"
      open={open}
