...
export function CanvasProjectSidebar({ projectId, detail, onAddChapter, onLocateStyle, onOpenAssets }: CanvasProjectSidebarProps) {
    ...
    const deferredQuery = useDeferredValue(query.trim().toLocaleLowerCase(formatLocale()));
    ...
    const visibleUnits = useMemo(() => {
        ...
        return orderedUnits.reduce<Array<{ unit: ProjectUnit; index: number }>>((result, unit, index) => {
            ...
            const matchesTitle = unit.title.toLocaleLowerCase(formatLocale()).includes(deferredQuery);
    ...
    if (collapsed) {
        return (
            <aside className="relative z-[var(--z-panel)] hidden w-11 shrink-0 flex-col items-center border-r border-border bg-background/94 py-2 backdrop-blur-xl lg:flex">
                <button
                    type="button"
                    className="grid size-7 place-items-center rounded-md text-foreground/55 hover:bg-surface-hover"
                    title={t("canvas:expand-project-sidebar")}
                    aria-label={t("canvas:expand-project-sidebar")}
                    onClick={() => setCollapsed(false)}
                >
                    <ChevronRight className="size-4" />
                </button>
                <Link to={`/projects/${projectId}/canvases`} className="mt-2 grid size-7 place-items-center rounded-md text-foreground/55 hover:bg-surface-hover" title={t("canvas:back-to-project-canvas-list")}>
    ...
    return (
        <aside className="relative z-[var(--z-panel)] hidden w-[var(--canvas-sidebar-width)] shrink-0 flex-col border-r border-border bg-background/94 backdrop-blur-xl lg:flex">
            <header className="flex h-12 shrink-0 items-center justify-between gap-2 border-b border-border px-2.5">
                <Link to={`/projects/${projectId}/canvases`} className="flex min-w-0 items-center gap-2 text-xs font-semibold" title={t("canvas:back-to-project-canvas-list")}>
                    <FolderKanban className="size-3.5 shrink-0" />
                    <span className="truncate">{projectDetail?.project.name || t("canvas:project-space")}</span>
                </Link>
                <button
                    type="button"
                    className="grid size-7 shrink-0 place-items-center rounded-md text-foreground/45 hover:bg-surface-hover"
                    title={t("canvas:collapse-project-sidebar")}
                    aria-label={t("canvas:collapse-project-sidebar")}
            ...
            <section className="shrink-0 border-b border-border/70 p-2">
                <div className="mb-1.5 flex h-5 items-center justify-between px-1 text-[var(--fs-label)] font-medium text-foreground/48">
                    <span className="flex items-center gap-1.5">
                        ...
                        {t("canvas:project-style-3")}
                    </span>
                    <Link
                        to={`/projects/${projectId}/settings`}
                        className="grid size-6 place-items-center rounded text-foreground/35 hover:bg-surface-hover hover:text-foreground"
                        title={t("canvas:edit-project-style")}
                        aria-label={t("canvas:edit-project-style")}
                    >
                ...
                <button
                    ...
                    title={style ? t("canvas:locate-the-project-style-node-on-canvas") : t("canvas:project-has-no-style-set-yet")}
                >
                    {style ? (
                    ...
                    <span className="min-w-0 flex-1">
                        <span className="block truncate text-xs font-medium">{style?.title || t("canvas:no-style-set-yet")}</span>
                        <span className="mt-0.5 block truncate text-[var(--fs-micro)] text-foreground/38">{style ? t("canvas:auto-synced-with-project-settings") : t("canvas:choose-in-project-settings")}</span>
                    </span>
            ...
            <section className="flex min-h-0 flex-1 flex-col">
                <div className="flex h-9 shrink-0 items-center justify-between px-3">
                    <span className="flex items-center gap-1.5 text-[var(--fs-label)] font-medium text-foreground/48">
                        ...
                        {t("canvas:story-chapters")} <span className="tabular-nums text-foreground/32">{orderedUnits.length.toLocaleString(formatLocale())}</span>
                    </span>
                    {selectedId ? (
                        <button
                            type="button"
                            onClick={revealSelectedChapter}
                            className="grid size-6 place-items-center rounded text-foreground/35 hover:bg-surface-hover hover:text-foreground"
                            title={t("canvas:back-to-current-chapter")}
                            aria-label={t("canvas:back-to-current-chapter")}
                        >
                ...
                <div className="shrink-0 px-2 pb-2">
                    <label className="flex h-8 items-center gap-1.5 rounded-md border border-border/75 bg-foreground/[.025] px-2 focus-within:border-[var(--workspace-accent)] focus-within:ring-2 focus-within:ring-[var(--workspace-accent-soft)]">
                        ...
                        <input
                            ...
                            placeholder={t("canvas:search-titles-or-type-a-chapter-number")}
                            aria-label={t("canvas:search-project-chapters")}
                        />
                        {query ? (
                            <button type="button" onClick={() => setQuery("")} className="grid size-5 shrink-0 place-items-center rounded text-foreground/32 hover:bg-surface-hover hover:text-foreground" aria-label={t("canvas:clear-chapter-search")}>
                                <X className="size-3" />
                            </button>
                        ) : null}
                    </label>
                    {deferredQuery ? (
                        <div className="mt-1 px-0.5 text-[var(--fs-micro)] tabular-nums text-foreground/35">
                            {t("canvas:found")} {visibleUnits.length.toLocaleString(formatLocale())} {t("canvas:item-6")}
                        </div>
                    ) : null}
                </div>

                <div ref={scrollRef} className="thin-scrollbar min-h-0 flex-1 overflow-y-auto overscroll-contain px-1.5" aria-label={t("canvas:project-chapter-list")}>
                    {visibleUnits.length ? (
                                    ...
                                    <div key={unit.id} className="absolute left-0 top-0 w-full" style={{ height: virtualItem.size, transform: `translateY(${virtualItem.start}px)` }}>
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                aria-label={t("canvas:preview-chapter-aria", { index: index + 1, title: unit.title })}
                                            >
                                            ...
                                            <button
                                                ...
                                                title={t("canvas:add-to-canvas-center")}
                                                aria-label={t("canvas:add-param-to-canvas", { title: unit.title })}
                        ...
                        <div className="px-2 py-8 text-center text-xs text-foreground/40">{orderedUnits.length ? t("canvas:no-matching-chapters") : t("canvas:no-chapters-yet")}</div>
                    )}
                </div>
            </section>
            ...
            <section className="shrink-0 border-t border-border/70 p-2">
                <button type="button" onClick={onOpenAssets} className="flex h-9 w-full items-center gap-2 rounded-md border border-border/45 px-2 text-xs text-foreground/65 hover:border-[var(--workspace-accent)] hover:bg-[var(--workspace-accent-soft)]">
                    ...
                    <span className="flex-1 text-left">{t("canvas:reference-project-assets-2")}</span>
                    <span className="rounded bg-foreground/[.06] px-1.5 py-0.5 text-[var(--fs-micro)] tabular-nums text-foreground/45">{mediaAssetCount.toLocaleString(formatLocale())}</span>
                </button>
            </section>
...
function ChapterPreview({
    ...
    return (
        <section
            ...
            aria-label={t("canvas:chapter-param-preview", { chapterNumber: chapterNumber })}
        >
            <header className="flex items-start gap-2 border-b border-border/70 px-3 py-2.5">
                <div className="min-w-0 flex-1">
                    <div className="text-[var(--fs-tiny)] tabular-nums text-foreground/38">
                        {t("canvas:chapter")} {chapterNumber.toLocaleString(formatLocale())} {t("canvas:item-6")}
                    </div>
                    <h2 className="mt-0.5 truncate text-sm font-semibold">{unit.title}</h2>
                </div>
                <button type="button" onClick={onClose} className="grid size-6 shrink-0 place-items-center rounded text-foreground/38 hover:bg-surface-hover hover:text-foreground" aria-label={t("canvas:close-chapter-preview")}>
                    <X className="size-3.5" />
                </button>
            </header>
            <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto px-3 py-3">
                <p className="whitespace-pre-wrap text-xs leading-5 text-foreground/58">
                    {loading ? t("canvas:loading-chapter-content") : loadError ? t("canvas:failed-to-read-content-try-again-later") : text || t("canvas:this-chapter-has-no-content-yet")}
                </p>
                {!loading && !loadError ? (
                    <div className="mt-2 text-[var(--fs-tiny)] tabular-nums text-foreground/35">
                        {text.length.toLocaleString(formatLocale())} {t("canvas:item-9")}
                    </div>
                ) : null}
            </div>
