...
export const CanvasFrameNode = React.memo(function CanvasFrameNode({
    ...
    const commitTitle = () => {
        const next = title.trim() || (folder ? t("domain:untitled-folder") : t("domain:untitled-frame"));
        setTitle(next);
        setEditing(false);
    ...
    return (
        <div
            ...
            aria-label={folder && collapsed ? t("domain:param-folder-param-items-press-enter-to-open", { title: data.title, length: childNodes.length }) : undefined}
                    ...
                    <>
                        <div className="pointer-events-auto absolute inset-x-0 top-0 z-10 flex items-center gap-1.5 px-1.5" style={{ height: FRAME_HEADER_HEIGHT, color: theme.node.text }}>
                            <button
                                ...
                                aria-label={collapsed ? t("domain:expand-frame") : t("domain:collapse-frame")}
                    ...
                    <div key={node.id} className="absolute overflow-hidden rounded-[var(--r-xs)] border" style={{ ...style, background: theme.node.fill, borderColor: theme.node.stroke }}>
                        ...
                        {node.type === CanvasNodeType.Script ? (
                            <div className="p-1 text-[var(--fs-nano)] leading-[9px]" style={{ color: theme.node.text }}>
                                {t("domain:storyboard-script")} {node.metadata?.storyboard?.rows.length || 0} {t("canvas:shots-2")}
                            </div>
                        ) : null}
                    </div>
                ))
...
function ResizeHandle({ corner, scale, theme, onMouseDown }: { corner: ResizeCorner; scale: number; theme: CanvasTheme; onMouseDown: (event: ReactMouseEvent, corner: ResizeCorner) => void }) {
    ...
    return (
        <button
            type="button"
            aria-label={t("domain:resize-frame")}
