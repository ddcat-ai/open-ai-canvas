...
export function CanvasShortDramaGuide({ progress, collapsed, onToggle, onSkip, onStepClick }: {
    ...
    if (collapsed) {
        return (
            <button type="button" data-canvas-no-zoom className="absolute left-[calc(50%+64px)] top-2 z-dock inline-flex h-8 max-w-[calc(50vw-76px)] min-w-0 items-center gap-2 rounded-lg border px-3 text-xs font-medium shadow-sm backdrop-blur outline-none focus-visible:ring-2" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text, "--tw-ring-color": theme.accent.primary } as CSSProperties} onClick={onToggle}>
    ...
    return (
        <div data-canvas-no-zoom className="absolute left-1/2 top-[68px] z-dock flex max-w-[calc(100%_-_24px)] -translate-x-1/2 items-center gap-1 rounded-lg border p-1 shadow-sm backdrop-blur" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text }}>
