...
import { CanvasVideoIcon } from "./canvas-workspace-asset-panel";

export function CanvasWorkspaceNodeListPanel({ nodes, results, query, deferredQuery, selectedNodeIds, onQueryChange, onFocus }: {
        ...
        >
            ...
            <span className="min-w-0 self-center">
                <span className="flex min-w-0 items-center gap-1.5">
                    <span className="min-w-0 truncate text-xs font-medium leading-4 text-foreground" title={node.title}>{node.title || getNodeListLabel(node.type)}</span>
                </span>
                <span className="mt-0.5 block truncate text-[10px] leading-3 text-foreground/45" title={materialSummary}>{materialSummary}</span>
                <span className="mt-0.5 flex min-w-0 items-center gap-1 text-[10px] leading-3 text-foreground/40">
                    ...
                    <span className="mt-0.5 block text-[10px] tabular-nums text-foreground/35" title={fullTime(times.updatedAt)}>{times.updatedLabel}</span>
                </span>
...
function CanvasNodeListThumbnail({ node }: { node: CanvasNodeData }) {
    ...
    if (textPreview && (node.type === CanvasNodeType.Text || node.type === CanvasNodeType.Markdown || node.type === CanvasNodeType.Script)) {
        return <span aria-hidden="true" className="line-clamp-3 h-9 w-11 overflow-hidden rounded-[var(--r-sm)] border px-1 py-0.5 text-[7px] leading-[10px] text-foreground/55" style={commonStyle}>{textPreview}</span>;
    }
    ...
    return (
        <span aria-hidden="true" className="grid h-9 w-11 place-items-center rounded-[var(--r-sm)] border text-foreground/48" style={commonStyle}>
            {node.type === CanvasNodeType.Image || node.type === CanvasNodeType.Panorama ? <Image className="size-3.5" />
                : node.type === CanvasNodeType.Video ? <CanvasVideoIcon className="size-3.5" />
                    : node.type === CanvasNodeType.Audio ? <AudioLines className="size-3.5" />
                        : node.type === CanvasNodeType.Drawing ? <Pencil className="size-3.5" />
