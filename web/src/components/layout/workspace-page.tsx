...
export function PaginationBar({ current, pageSize, total, onChange, pageSizeOptions = [20, 50, 100] }: { current: number; pageSize: number; total: number; onChange: (page: number, pageSize: number) => void; pageSizeOptions?: number[] }) {
    ...
    return (
        <div className="app-pagination-bar mt-4 flex min-h-10 min-w-0 items-center justify-end gap-2 px-2 py-1.5">
            <span className="app-pagination-total">{start}-{end} / 共 {total} 条</span>
            <Select
                size="small"
                value={pageSize}
