...
export function useExternalAssetSources(open: boolean): ExternalAssetSourceState {
        ...
        void Promise.allSettled(
            sources.map(async (source) => {
                try {
                    ...
                    const detail = reason instanceof Error ? reason.message : t("domain:failed-to-read");
            ...
            .then((results) => {
                ...
                results.forEach((result) => {
                    ...
                    else errors.push(result.reason instanceof Error ? result.reason.message : t("domain:failed-to-read-external-asset-sources"));
                });
    ...
    const importExternalAsset = useCallback(
        async (reference: ExternalAssetPickerReference, signal?: AbortSignal) => {
            ...
            if (!source?.provider.importAsset) throw new Error(t("domain:param-does-not-support-importing-project-assets-yet", { sourceName: reference.sourceName }));
    ...
    const uploadExternalFiles = useCallback(
        async (files: FileList | File[], folderId?: string, signal?: AbortSignal) => {
            ...
            if (!source?.provider.uploadFile) throw new Error(t("domain:the-current-plugin-does-not-support-writing-files"));
...
function resolveUploadSource(sources: HostedAssetSource[], folderId?: string) {
    ...
    throw new Error(t("domain:choose-a-plugin-folder-before-writing-assets"));
}
...
function toPickerItem(source: HostedAssetSource, item: ExternalAssetItem): AssetLibraryPickerItem {
    ...
    return {
        ...
        kindLabel: item.kind === "video" ? t("domain:video") : item.kind === "audio" ? t("domain:audio") : t("domain:image"),
