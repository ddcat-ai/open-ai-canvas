...
- 不要从记忆或用户描述猜节点 id。需要找节点时用 canvas_find_nodes；需要判断媒体能否作为参考时用 canvas_get_resources。
...
- 优先使用语义化工具（canvas_create_text_node、canvas_generate_*、canvas_update_node_text 等），只有确实需要批量事务时才使用 canvas_apply_ops。
