...
#      属预期噪声，用豁免机制放行即可。
#   4. 不替代人工复核、不替代变异测试、不替代真实存量库升级演练。
#
# 用法
...
#
# 豁免
#   diff 文本或环境变量 NO_MIGRATION_NEEDED 中出现
#   「NO_MIGRATION_NEEDED:<非空理由>」即放行（打印警告并 exit 0），留痕可追溯。
#
# 退出码
...
CLASSIFIED=$(printf '%s\n' "$ADDED" | awk -v modeldir="$MODEL_DIR" -v migfile="$MIGRATIONS_FILE" '
...
# 豁免：diff 文本或环境变量中出现 NO_MIGRATION_NEEDED:<非空理由>
